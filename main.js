...
// Resolve the app icon — proper multi-resolution .ico for Windows, .png for Linux/Mac
// assets/icon.ico is generated by `node scripts/create-icon.js` from the app logo
const appIconPath = process.platform === 'win32'
    ? path.join(__dirname, 'assets', 'icon.ico')
    : path.join(__dirname, 'assets', 'icon.png');

// CRITICAL for Windows: set App User Model ID so Windows associates
// the taskbar button with this app ID and shows the correct icon.
// Must be called before the first BrowserWindow is created.
if (process.platform === 'win32') {
    app.setAppUserModelId('com.bookmaster.pos');
}
...
function createWindow() {
    ...
    // Explicitly set the window icon after creation (reinforces taskbar icon on Windows)
